...
export default async function DocsHomePage({
    ...
    const itemsAll = listSectionIndex("docs");
    const q = (searchParams?.q ?? "").toLowerCase();
    const tree = listSectionFolderTree("docs");
    ...
    function collectFiles(
        ...
        const files: { href: string; title: string; description?: string }[] =
            [];
    ...
    const sections: {
        label: string;
        files: { href: string; title: string; description?: string }[];
    }[] = [];
    if (rootFiles.length) {
        sections.push({
            label: "General",
